...
%
 bc = [];
 bc = pside(bc, G, 'YMin', 100*barsa, 'sat', [1 0]);
% 
...
for w = 1 : numel(wtype),
   W = verticalWell(W, G, rock, wloc(1,w), wloc(2,w), [], ...
                    ...
                    'Sign', sgn(w), 'InnerProduct', 'ip_tpf','Comp_i' , [0, 1, 0]);
end
% figure
...
schedule = simpleSchedule(dt, 'Well',W,'bc', bc);
%pause
...
try
    ...
    %pressureSolver = GMRES_ILUSolverAD('tolerance', 1e-2,'maxIterations', 1000);
catch
  %  pressureSolver = AGMGSolverAD('tolerance', 1e-4);
    %pressureSolver = BackslashSolverAD();
end
